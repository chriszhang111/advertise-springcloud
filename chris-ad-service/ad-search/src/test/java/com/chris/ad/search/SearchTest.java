package com.chris.ad.search;


import com.alibaba.fastjson.JSON;
import com.chris.ad.Application;
import com.chris.ad.search.vo.SearchRequest;
import com.chris.ad.search.vo.feature.DistrictFeature;
import com.chris.ad.search.vo.feature.FeatureRelation;
import com.chris.ad.search.vo.feature.ItFeature;
import com.chris.ad.search.vo.feature.KeywordFeature;
import com.chris.ad.search.vo.media.AdSlot;
import com.chris.ad.search.vo.media.App;
import com.chris.ad.search.vo.media.Device;
import com.chris.ad.search.vo.media.Geo;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.test.context.junit4.SpringRunner;

import java.util.Arrays;
import java.util.Collections;
import java.util.List;

@RunWith(SpringRunner.class)
@SpringBootTest(classes = {Application.class}, webEnvironment = SpringBootTest.WebEnvironment.NONE)
public class SearchTest {

    @Autowired
    private ISearch search;

    @Test
    public void testFetchAds(){
        SearchRequest request = new SearchRequest();
        request.setMediaId("imooc-ad");

        //1
        request.setRequestInfo(new SearchRequest.RequestInfo(
                "id",
                Collections.singletonList(
                        new AdSlot(
                                "ad-x", 1, 1080, 720, Arrays.asList(1,2), 1000
                        )
                ),
                buildsampleApp(),
                buildGeo(),
                buildDevice()
        ));

        request.setFeatureInfo(buildFeature(
                Arrays.asList("宝马", "大众"),
                Collections.singletonList(new DistrictFeature.ProvinceAndCity(
                        "安徽省","合肥市"
                )),
                Arrays.asList("台球", "游泳"),
                FeatureRelation.OR
        ));

        System.out.println(JSON.toJSONString(
                search.fetchAds(request)
        ));

    }

    private App buildsampleApp(){
        return new App("imooc", "imooc", "imooc", "imooc");
    }

    private Geo buildGeo(){
        return new Geo((float)0.0, (float)0.0, "Beijing", "Beijing");
    }

    private Device buildDevice(){
        return new Device();
    }

    private SearchRequest.FeatureInfo buildFeature(
            List<String> keywords,
            List<DistrictFeature.ProvinceAndCity> provinceAndCities,
            List<String> its,
            FeatureRelation relation
    ){

        return new SearchRequest.FeatureInfo(
                new KeywordFeature(keywords),
                new DistrictFeature(provinceAndCities),
                new ItFeature(its),
                relation
        );
    }

}
